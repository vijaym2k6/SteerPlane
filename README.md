...
**Security model (today):** The agent points its OpenAI client at the gateway and passes the real provider key in the `X-LLM-API-Key` header. The gateway authenticates the SteerPlane key, runs every request through enforcement (policy → cost → loop), and only then forwards it upstream with that provider key — so the agent can't reach the provider directly or bypass the guardrails. The provider key is used transiently per request and is **not** persisted by SteerPlane. Server-side key vaulting (so the agent never transmits the provider key at all) is on the roadmap.
